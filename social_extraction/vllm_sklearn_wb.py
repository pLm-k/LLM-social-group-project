    ...
    with wandb.init(config=config):
        ...
        for l, p in zip(label, pre):
            if len(l.split(" ")) == len(p.split(" ")):
                ...
                # print ("\n >>>prediction: \n", p)
                # print(p.split(" "))
        print("prediction_list:", prediction_list, "label_list:", label_list)
        ...
        wandb.log({"f1": f1_score(convert_to_bio_format(np.array(label_list)).tolist(), convert_to_bio_format(np.array(prediction_list)).tolist(), average='weighted')})
        ray.shutdown()
